<p align="center">
  ...
  <h1 align="center">react-native-alipay-v5</h1>
<p>

> **仓库开发说明**：本仓库基于react-native-alipay开发，感谢原作者 [@uiwjs]及 [Kenny Wong]。因原包存在模拟器调试链接缺陷，本 Fork 在原有基础上进行了 iOS XCFramework 升级， 以支持模拟器调试。支持Expo。

[![NPM Version](https://img.shields.io/npm/v/react-native-alipay-v5.svg)](https://npmjs.org/package/react-native-alipay-v5)

基于 React Native 的支付宝插件，支持 iOS/Android（iOS 支持 M 系列芯片模拟器）。适用于商家在 App 应用中集成支付宝支付功能，商家 APP 调用支付宝提供的 SDK，SDK 再调用支付宝 APP 内的支付模块。如果用户已安装支付宝APP，商家APP会跳转到支付宝中完成支付，支付完后跳回到商家 APP 内，最后展示支付结果。如果用户没有安装支付宝 APP，商家 APP 内会调起支付宝网页支付收银台，用户登录支付宝账户，支付完后展示支付结果。完整实例 [Example](./example) | [完整的接口文档](https://uiwjs.github.io/react-native-alipay/)
...
**v5 更新说明：**
- 包名改为 `react-native-alipay-v5`
- iOS SDK 升级为 XCFramework 格式，支持 M 系列芯片ios模拟器iphone simulator
- Podspec 文件更名为 `react-native-alipay-v5.podspec`
...
7. iOS 使用 XCFramework 格式的 AlipaySDK，支持 arm64 真机及 arm64/x86_64 模拟器（含 M 系列芯片iphone simulator）。
8. `URL Schemes` 要以字母开头不能为纯数字。

## 安装依赖

```bash
yarn add react-native-alipay-v5
...
```javascript
import Alipay from 'react-native-alipay-v5';
...
```bash
npx create-react-native-module --package-identifier com.uiwjs --object-class-name RNAlipay --generate-example Alipay --example-react-native-version 0.63.0 --module-name react-native-alipay-v5 --github-account uiwjs --author-name "Kenny Wong" --author-email "wowohoo@qq.com"
```
...
- [本仓库 react-native-alipay-v5](https://github.com/zeeklog/react-native-alipay-v5)
- [支付宝：生成秘钥指南](https://opendocs.alipay.com/open/291/105971)
- [支付宝：App 支付客户端 DEMO&SDK 下载（XCFramework 格式）](https://opendocs.alipay.com/common/02km9l)
...
- [react-native-wechat-v2](https://github.com/zeeklog/react-native-wechat-v2) 微信支付SDK。
